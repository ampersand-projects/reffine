...
IterSpace ReffinePass::Build(Op& op)
{
    ...
    ispace.idx_to_iter = [](Expr idx) { return make_shared<Cast>(types::INT64, idx); };
    ispace.iter_to_idx = [](Expr iter) { return make_shared<Cast>(types::IDX, iter); };
    ispace.idx_incr = [](Expr idx) { return make_shared<Add>(idx, make_shared<Const>(BaseType::IDX, 1)); };
