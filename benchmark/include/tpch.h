...
struct TPCHQuery6 {
    ...
    TPCHQuery6(int64_t start, int64_t end, double disc, double quant)
    {
        ...
        this->query6_fn = compile_op<void(*)(double*, ArrowTable*)>(this->build_op(tbl, start, end, disc, quant));
    }

    shared_ptr<Func> build_op(shared_ptr<ArrowTable2> tbl, int64_t start, int64_t end, double disc, double quant)
    {
        auto vec_in_sym = _sym("vec_in", tbl->get_data_type());
        auto red = _red(_subvec(vec_in_sym, _idx(0), _len(vec_in_sym, 1)),
            [](){ return _f64(0); },
            [start, end, disc, quant](Expr s, Expr v){
                ...
                auto date_pred = _gte(l_shipdate, start_date) & _lt(l_shipdate, end_date);
                auto discount_pred = _gte(l_discount, _sub(discount, _f64(0.01))) & _lte(l_discount, _add(discount, _f64(0.01)));
